...
namespace FMODUnity
{
    public class AsyncBankLoadHandle
    {
    ...
    public class RuntimeManager : MonoBehaviour
    {
       ...
       // private readonly Stack<AppState> pendingStackedPauseOperations = new Stack<AppState>();
        ...
        //  public static RuntimeManager CreateInstanceIfNull()
        //{
        //    return Instance;
        //}

        public void CreateInstanceOfRunTimeManager()
        {
            StartCoroutine(CreateInstanceOfRunTimeManagerRoutine());
        }

        private IEnumerator CreateInstanceOfRunTimeManagerRoutine()
        {
            yield return null;
        ...
        private FMOD.RESULT Initialize()
        {
            ...
            StartCoroutine(LoadBanks(fmodSettings));
        ...
        private void PauseDetectorTask()
        {
            //  Debug.Log("Task started");
            AndroidJNI.AttachCurrentThread();
            bool lockIsBeingHeld = false;
            ...
            {
                print("Error occurred while executing pause detector task " + e);
            }
            finally
            {
                if(lockIsBeingHeld)
        ...
        public static AsyncBankLoadHandle LoadBank(string bankName, bool loadSamples = false, FMOD.Studio.LOAD_BANK_FLAGS loadBankFlags = FMOD.Studio.LOAD_BANK_FLAGS.NORMAL)
        {
            AsyncBankLoadHandle handle = new AsyncBankLoadHandle();
            ...
            else
            {
                ...
                // #endif // (UNITY_ANDROID || UNITY_WEBGL) && !UNITY_EDITOR
                {
                    ...
                    Instance.StartCoroutine(LoadTheBankRoutine(handle, loadedBank, loadBankFlags, bankPath, bankName, loadSamples));
        ...
        private static IEnumerator LoadTheBankRoutine(AsyncBankLoadHandle handle, LoadedBank loadedBank, LOAD_BANK_FLAGS loadBankFlags, string bankPath, string bankName, bool loadSamples)
        {
            FMOD.RESULT loadResult = Instance.studioSystem.loadBankFile(bankPath, loadBankFlags, out loadedBank.Bank);

            LOADING_STATE lOADING_STATE;

            FMOD.RESULT result = loadedBank.Bank.getLoadingState(out lOADING_STATE);
            while (lOADING_STATE != LOADING_STATE.LOADED && lOADING_STATE != LOADING_STATE.ERROR)
            {
                loadedBank.Bank.getLoadingState(out lOADING_STATE);
        ...
        private IEnumerator LoadBanks(Settings fmodSettings)
        {
            if (fmodSettings.ImportType == ImportType.StreamingAssets)
            {
                ...
                //  try
                //  {
                List<AsyncBankLoadHandle> handles = new List<AsyncBankLoadHandle>();
                foreach (string bankName in BanksToLoad(fmodSettings))
                {
                    handles.Add(LoadBank(bankName, false, FMOD.Studio.LOAD_BANK_FLAGS.NONBLOCKING));
                }

                bool isLoading = true;
                while (isLoading)
                {
                    isLoading = !handles.All((x) => x.isDone);
