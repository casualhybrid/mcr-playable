    ...
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        internal void InitialiseChildObjects()
        {
            if (ChildObjects != null && ChildObjects.Length > 0)
            {
                ...
                {
                    UnityEngine.Console.LogError("ScrollRect Content has children, this is not supported when using managed Child Objects\n Either remove the ScrollRect Content children or clear the ChildObjects array");
                    return;
                }
        ...
        internal void UpdateVisible()
        {
            ...
            for (int i = CurrentPage - _bottomItem; i < CurrentPage + _topItem; i++)
            {
                ...
                {
                    UnityEngine.Console.Log("Failed to setactive child [" + i + "]");
                }
            }
        ...
        private void OnValidate()
        {
            ...
            {
                UnityEngine.Console.LogError("ScrollRect has to have a direction, please select either Horizontal OR Vertical with the appropriate control.");
            }
            if (_scroll_rect.horizontal && _scroll_rect.vertical)
            {
                UnityEngine.Console.LogError("ScrollRect has to be unidirectional, only use either Horizontal or Vertical on the ScrollRect, NOT both.");
            }
            ...
            {
                UnityEngine.Console.LogError($"[{gameObject.name}]When using procedural children with a ScrollSnap (Adding Prefab ChildObjects) and the Infinite Scroll component\nYou must set the 'InitByUser' option to true, to enable late initialising");
            }
        }
