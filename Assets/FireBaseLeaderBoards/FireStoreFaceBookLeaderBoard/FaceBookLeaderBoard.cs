...
namespace TheKnights.LeaderBoardSystem.FaceBook
{
    ...
    public class FaceBookLeaderBoard : ScriptableObject
    {
        ...
        public async Task SubmitHighScoreToLeaderBoard(int score, byte[] profilePicBytes = null)
        {
            ...
            if (!FireBaseAuthentication.IsLoggedInToFireBase)
            {
                UnityEngine.Console.Log("Failed to submit score to facebook leaderboard ad the user isn't logged in to facebook");
                return;
            }

            // Update the score locally
            if (CachedLeaderBoardData != null && CachedLeaderBoardData.isValid)
            {
                int cachedScore = CachedLeaderBoardData.LocalPlayerData.score;
                if (score > cachedScore)
                {
                    CachedLeaderBoardData.LocalPlayerData.score = score;
                    CachedLeaderBoardData.Data.Sort((a, b) => b.CompareTo(a));
                }
            }
