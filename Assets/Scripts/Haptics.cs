...
namespace RDG
{
    ...
    public static class Haptics
    {
        ...
        public static void VibrateAccordingToConfig(HapticsConfig hapticsConfig)
        {
            if (hapticsConfig.GetIsThisRepeatingEffect)
            {
                Vibrate(hapticsConfig.GetMilliSecondsList, hapticsConfig.GetAmplitudesList, hapticsConfig.GetRepeatIndexValue,
                    hapticsConfig.GetCancelPrevious);
            }
            else
            {
                Vibrate(hapticsConfig.GetMilliSecond, hapticsConfig.GetAmplitude, hapticsConfig.GetCancelPrevious);
            }
        }
        ...
        public static void Vibrate (long milliseconds, int amplitude = -1, bool cancel = false)
        {
            if (milliseconds <= 0)
            {
                UnityEngine.Console.LogError("Cannot give milliseconds value of less than 1 to Vibrate function");
                return;
            }
        ...
        public static void Vibrate (long[] pattern, int[] amplitudes = null, int repeat = -1, bool cancel = false)
        {
            ...
            {
                UnityEngine.Console.LogError("Cannot give empty milliSecondsList to Vibrate function");
                return;
            }
        ...
        private static void logAuto (string text, logLevel level)
        {
            ...
            if (text != null) {
                if (level == logLevel.Warning && LogLevel == logLevel.Warning) {
                    UnityEngine.Console.LogWarning(text);
                }
                else if (level == logLevel.Info && LogLevel >= logLevel.Info) {
                    UnityEngine.Console.Log(text);
                }
            }
        }
