...
namespace deVoid.UIFramework
{
        ...
        where TProps : IScreenProperties
    {
        ...
        public UIScreenEvents UIScreenEvents { get; set; }

        public void OpenThePanel(string id)
        {
            UIScreenEvents.RaisePanelOpenEvent(id);
        }

        public void OpenThePanel(string id, IPanelProperties properties = null)
        {
            UIScreenEvents.RaisePanelOpenEvent(id, properties);
        }

        public void CloseThePanel(string id)
        {
            UIScreenEvents.RaisePanelCloseEvent(id);
        }
        ...
        public void OpenTheWindow(string id, IWindowProperties properties = null)
        {
            UIScreenEvents.RaiseWindowOpenEvent(id, properties);
        }

        public void CloseTheWindow(string id)
        {
            UIScreenEvents.RaiseWindowCloseEvent(id);
        }
        ...
        public void Show(IScreenProperties props = null)
        {
            if (props != null)
            {
                ...
                {
                    string msg = "Properties passed have wrong type! (" + props.GetType() + " instead of " +
                                   typeof(TProps) + ")";
        ...
        private void DoAnimation(ATransitionComponent caller, Action callWhenFinished, bool isVisible)
        {
            ...
            if (caller == null)
            {
                ...
                {
                    //  UnityEngine.Console.Log($"Opened Screen {ScreenId} ");
                    UIScreenEvents.OnScreenOperationEventBeforeAnimation.Invoke(ScreenId, ScreenOperation.Open, screenType);
                    UIScreenEvents.OnScreenOperationEventAfterAnimation.Invoke(ScreenId, ScreenOperation.Open, screenType);

                }
                else if (!isVisible/* && gameObject.activeSelf*/)
                {
                    // UnityEngine.Console.Log($"Closed Screen {ScreenId} ");
                    UIScreenEvents.OnScreenOperationEventBeforeAnimation.Invoke(ScreenId, ScreenOperation.Close, screenType);
            ...
            {
                Action action = callWhenFinished;
                ...
                caller.Animate(transform, action);
            }
        }
        ...
        {
            UnityEngine.Console.LogError($"Hiding the screen {name} forcefully. This should not happen");
            gameObject.SetActive(false);
        }
    }
}