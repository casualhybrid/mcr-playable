        ...
        public override void Initialize(){
			...
			if (!string.IsNullOrEmpty(_mergeToFile) && _whitelistedCollections.Count > 1){
				...
				try {
					...
					UnityEngine.Console.Log(log, AssetDatabase.LoadAssetAtPath<ShaderVariantCollection>(file));
				} catch (System.Exception ex){
					UnityEngine.Console.LogError("Error merging ShaderVariantCollections. Exception follows.");
					throw;
				}
			}
