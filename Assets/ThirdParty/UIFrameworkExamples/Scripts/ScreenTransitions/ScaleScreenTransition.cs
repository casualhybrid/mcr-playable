    ...
    public class ScaleScreenTransition : ATransitionComponent
    {
        ...
        public override void Animate(Transform target, Action callWhenFinished) {
            ...
            if (doFade) {
                ...
                canvasGroup.DOFade(isOutAnimation ? 0f : 1f, 1).From(isOutAnimation ? 1f : 0f).SetEase(ease).SetUpdate(true);// * fadeDurationPercent);
            }
            ...
            if (isOutAnimation) {
               ...
               rTransform.transform.DOScale(new Vector3(0, 0, 0), duration)
                .From(new Vector3(1, 1, 1), true).SetEase(Ease.OutQuad).OnComplete(() => Cleanup(callWhenFinished, rTransform, canvasGroup))
                .SetUpdate(true);
               
            }
            else
            {
                rTransform.transform.DOScale(new Vector3(1, 1, 1), duration)
                .From(new Vector3(0, 0, 0), true).SetEase(Ease.OutQuad).OnComplete(() => Cleanup(callWhenFinished, rTransform, canvasGroup))
                .SetUpdate(true);
