...
namespace UnityEngine.UI.Extensions
{
        ...
        where TContext : class, IFancyScrollRectContext, new()
    {
        ...
        protected virtual void OnValidate()
        {
            ...
            {
                loop = false;
                UnityEngine.Console.LogError("Loop is currently not supported in FancyScrollRect.");
            }

            if (Scroller.SnapEnabled)
            {
                Scroller.SnapEnabled = false;
                UnityEngine.Console.LogError("Snap is currently not supported in FancyScrollRect.");
            }

            if (Scroller.MovementType == MovementType.Unrestricted)
            {
                Scroller.MovementType = MovementType.Elastic;
                UnityEngine.Console.LogError("MovementType.Unrestricted is currently not supported in FancyScrollRect.");
            }
        }
    }
