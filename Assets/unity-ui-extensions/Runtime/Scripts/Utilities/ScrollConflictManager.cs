        ...
        void Awake()
        {
            ...
            if (scrollOtherHorizontally)
            {
                if (_myScrollRect.horizontal)
                    UnityEngine.Console.LogError("You have added the SecondScrollRect to a scroll view that already has both directions selected");
                if (!ParentScrollRect.horizontal)
                    UnityEngine.Console.LogError("The other scroll rect does not support scrolling horizontally");
            }
            else if (!ParentScrollRect.vertical)
            {
                UnityEngine.Console.LogError("The other scroll rect does not support scrolling vertically");
            }
