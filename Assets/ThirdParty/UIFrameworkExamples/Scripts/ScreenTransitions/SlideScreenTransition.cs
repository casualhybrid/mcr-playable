...
namespace deVoid.UIFramework.Examples
{
    public class SlideScreenTransition : ATransitionComponent
    {
        ...
        private void Awake()
        {
            audioPlayerObj = gameObject.GetComponent<AudioPlayer>();
            origAnchoredPos = GetComponent<RectTransform>().anchoredPosition;
        }
        ...
        public override void Animate(Transform target, Action callWhenFinished) {
            ...
            if (doFade) {
                ...
                canvasGroup.DOFade(isOutAnimation ? 0f : 1f, duration).From(isOutAnimation ? 1f : 0f).SetEase(ease).SetUpdate(true);// * fadeDurationPercent);
            }
